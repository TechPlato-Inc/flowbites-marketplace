...
} from "@/modules/templates/services/templates.service";
import { RelatedTemplates } from "@/components/templates/RelatedTemplates";
import { ReviewList, ReviewForm } from "@/modules/reviews/components";
...
const licenseInfo: Record<string, { label: string; desc: string }> = {
  personal: {
    label: "Personal",
    desc: "For personal and non-commercial use only",
  },
  commercial: {
    label: "Commercial",
    desc: "Use in commercial projects, unlimited end products",
  },
  extended: {
    label: "Extended",
    desc: "Unlimited use, including SaaS and resale products",
  },
};
...
export function TemplateDetailView({ template }: TemplateDetailViewProps) {
  ...
  const allImages = [template.thumbnail, ...(template.gallery || [])].filter(
    Boolean,
  );
  ...
  const license =
    licenseInfo[template.licenseType || "personal"] || licenseInfo.personal;
  const creator =
    typeof template.creatorProfileId === "object"
      ? template.creatorProfileId
      : null;
  const creatorName = creator?.displayName || "Creator";
  const creatorUsername = creator?.username;
  const creatorAvatar = creator?.avatar;
  const creatorLink = creatorUsername
    ? `/creators/${creatorUsername}`
    : creator?._id
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 pt-6 pb-2">
        ...
        >
          ...
          {typeof template.category === "object" && template.category?.name && (
            <>
              <Link
                href={`/templates?category=${template.category._id}`}
                className="hover:text-neutral-900 transition-colors"
              >
          ...
          <div className="lg:col-span-2">
            ...
            <div className="mb-6">
              <div className="flex flex-wrap items-center gap-2 mb-3">
                <span
                  className={`inline-flex items-center px-2.5 py-0.5 text-xs font-semibold rounded-full ${platform.color}`}
                >
                  {platform.label}
                </span>
              ...
              <div className="flex items-center gap-3">
                ...
                {creator?.isVerified && (
                  <span className="inline-flex items-center gap-1 text-xs text-primary-600">
                    <CheckCircle size={14} className="fill-primary-100" />{" "}
                    Verified
                  </span>
                )}
              </div>
            ...
            <div className="flex flex-wrap items-center gap-6 py-5 border-y border-neutral-200 mb-8">
              <div className="flex items-center gap-2 text-sm text-neutral-600">
                ...
                <span className="font-medium">
                  {(template.stats?.views || 0).toLocaleString()}
                </span>
            ...
            {!servicesLoading && (
              <div
                className="mt-4 pt-8 border-t border-neutral-200"
                ...
                <div className="flex items-center gap-2.5 mb-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-neutral-500">
                      Professional customization services available
                    </p>
                  </div>
                </div>
                ...
                {services.length > 0 ? (
                  ...
                  <div className="border border-dashed border-neutral-300 rounded-xl p-6 bg-neutral-50/50">
                    <p className="text-neutral-600 mb-4">
                      The creator doesn&apos;t offer customization services for
                      this template yet. Our <strong>Flowbites team</strong> can
            ...
            >
                ...
                <div>
                  ...
                  {template.stats?.averageRating && (
                    <div className="flex items-center gap-1 text-sm text-neutral-500">
                      ...
                      <span>({template.stats.totalReviews || 0} reviews)</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              {isAuthenticated && (
                <div className="mb-6">
                  <ReviewForm templateId={template._id} />
                </div>
              )}

              <ReviewList templateId={template._id} />

              {/* Version History */}
              <div className="mt-8 pt-8 border-t border-neutral-200">
                <VersionHistory templateId={template._id} />
              </div>
            </div>
          </div>
            ...
            <div className="lg:sticky lg:top-20 space-y-4">
              ...
              <div className="border border-neutral-200 rounded-xl p-5 sm:p-6 bg-white">
                <div className="flex items-center justify-between mb-5">
                  ...
                  {isAuthenticated && (
                    <div className="flex items-center gap-2">
                      <WishlistButton templateId={template._id} size="md" />
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-3 text-sm">
                  <div className="flex items-center justify-between py-2 border-t border-neutral-100">
                    ...
                    <span
                      className={`inline-flex items-center px-2 py-0.5 text-xs font-semibold rounded-full ${platform.color}`}
                    >
                      {platform.label}
                    </span>
                  </div>
              ...
              <div className="border border-neutral-200 rounded-xl p-5 bg-white">
                ...
                >
                  ...
                  <ArrowRight
                    size={16}
                    className="ml-auto text-neutral-300 group-hover:text-primary-500 transition-colors shrink-0"
                  />
                </Link>
                {creator?.stats && (
              ...
              <div className="border border-neutral-200 rounded-xl p-5 bg-white">
                <button
                  onClick={handleShare}
                  className="w-full flex items-center justify-center gap-2 py-2 text-sm text-neutral-600 hover:text-neutral-900 font-medium transition-colors"
                >
                  {copied ? (
                    <>
      ...
      <Modal
        ...
        title={requestSuccess ? "Request Submitted" : "Request Customization"}
        size="md"
        footer={
          requestSuccess ? (
            ...
            <div className="flex gap-3">
              ...
              <Button
                ...
                disabled={!requestRequirements.trim()}
              >
                Submit Request
              </Button>
            </div>
        ...
        {requestSuccess ? (
          <div className="text-center py-4">
            ...
            <p className="text-neutral-600">
              The Flowbites team will review your request and get back to you
              within 24 hours.
            </p>
          </div>
        ) : (
          <div className="space-y-4">
            <div className="bg-primary-50 rounded-lg p-3 text-sm">
              ...
              <p className="text-primary-700 mt-1">
                Our team will review your requirements and assign an experienced
                creator or handle it in-house.
              </p>
            </div>
            <div>
