...
interface TemplateStats {
  ...
  recentActivity: {
    ...
    status: string;
    createdAt: string;
    creatorProfileId?: { displayName: string };
  }[];
...
export const TemplateManager = () => {
  ...
  const handleReject = async () => {
    ...
    try {
      await api.post(`/admin/templates/${rejectTargetId}/reject`, {
        reason: rejectReason,
      });
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (activeView === "detail" || activeView === "edit")
  ...
  const handleBulkAction = async (action: string) => {
    ...
    try {
      ...
      showSuccess(
        `Bulk ${action}: ${data.data.success} succeeded, ${data.data.failed} failed`,
      );
  ...
  const handleBulkReject = async () => {
    ...
    try {
      ...
      showSuccess(
        `Bulk reject: ${data.data.success} succeeded, ${data.data.failed} failed`,
      );
      setSelectedIds(new Set());
  ...
  const handleExport = async () => {
    try {
      ...
      const response = await api.get(
        `/admin/templates/export?${params.toString()}`,
        {
          responseType: "blob",
        },
      );
  ...
  const navItems: NavItem[] = [
    ...
    },
    {
      ...
      badge: stats?.byStatus?.pending || 0,
      section: "Templates",
    },
    {
      label: "Featured",
      icon: Star,
      onClick: () => {
        setFilters({ ...DEFAULT_FILTERS, featured: "true" });
        setActiveView("templates");
      },
      badge: stats?.featuredCount || 0,
      section: "Templates",
    },
    {
      label: "Categories",
      ...
      <>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
          {statCards.map((s) => (
            ...
            >
              <div
                className={`w-8 h-8 rounded-lg flex items-center justify-center mb-3 ${s.color}`}
              >
                <s.icon size={16} />
              </div>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-neutral-400">
                      {t.stats.purchases} purchases
                    </div>
                  </div>
                  <span className="text-sm font-semibold text-neutral-900">
                    {formatCurrency(t.stats.revenue)}
                  </span>
                </button>
              ))}
            </div>
              ...
              <div className="flex gap-1">
                {PLATFORM_PILLS.map((p) => (
                  <button
                    key={p.value}
                    onClick={() =>
                      setFilters((f) => ({ ...f, platform: p.value }))
                    }
            ...
            >
              ...
              <div className="col-span-3 flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <button
                    onClick={() => openDetail(t._id)}
                    className="text-sm font-medium text-neutral-900 truncate block hover:text-primary-500 text-left"
                  >
                    {t.title}
                  </button>
              ...
              <div className="col-span-2 hidden md:block">
                {typeof t.creatorProfileId === "object" &&
                t.creatorProfileId ? (
                  <Link
              ...
              <div className="col-span-2 flex items-center gap-1">
                ...
                >
                  {t.isFeatured ? (
                    ...
                    <StarOff size={14} className="text-neutral-400" />
                  )}
                </button>
                {t.status === "pending" && (
                  <>
                    <button
                      ...
                      className="p-1.5 hover:bg-success-light rounded-lg"
                      title="Approve"
                    >
              ...
              <div className="flex items-center justify-between">
                ...
                >
                  {t.isFeatured ? (
                    ...
                    <StarOff size={16} className="text-neutral-300" />
                  )}
                </button>
              </div>
              {t.demoUrl && (
            ...
            <div className="bg-white border border-neutral-200 rounded-xl p-5">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  {
                    label: "Revenue",
                    value: formatCurrency(t.stats?.revenue || 0),
                  },
                  ...
                  <div className="flex gap-4 mt-3 pt-3 border-t border-neutral-100 text-xs text-neutral-500">
                    <span>
                      <strong className="text-neutral-700">
                        {t.creatorProfileId.stats.templateCount}
                      </strong>{" "}
                      templates
                    </span>
                    <span>
                      <strong className="text-neutral-700">
                        $
                        {(t.creatorProfileId.stats.totalRevenue || 0).toFixed(
                          0,
                        )}
                      </strong>{" "}
                      revenue
                    ...
                    <span>
                      <strong className="text-neutral-700">
                        {t.creatorProfileId.stats.totalSales || 0}
                      </strong>{" "}
                      sales
                    </span>
                  </div>
                ...
                {t.creatorProfileId.onboarding && (
                  <div className="mt-2">
                    <Badge
                      size="sm"
                      variant={
                        t.creatorProfileId.onboarding.status === "approved"
                          ? "success"
                          : t.creatorProfileId.onboarding.status === "submitted"
                            ? "warning"
                            : t.creatorProfileId.onboarding.status ===
                                "rejected"
                              ? "error"
                              : "neutral"
                      }
                    >
                      {t.creatorProfileId.onboarding.status?.replace(
                        /_/g,
                        " ",
                      ) || "unknown"}
                    </Badge>
          ...
          <div className="space-y-5">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setEditForm((f) => ({ ...f, licenseType: e.target.value }))
                }
                className="w-full border border-neutral-200 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500/20"
              >
      ...
      <div className="bg-white border border-neutral-200 rounded-xl overflow-hidden">
        ...
        {categoriesLoading ? (
          ...
          categories.map((c, i) => (
            <div
              key={c._id}
              className="grid grid-cols-12 gap-3 px-4 py-3 border-b border-neutral-50 last:border-0 hover:bg-neutral-50/50 items-center"
            >
    ...
    <>
      ...
      >
        <p className="text-sm text-neutral-600">
          Are you sure you want to permanently delete this template? This action
          cannot be undone.
        </p>
      </Modal>
      ...
      >
        <div>
          <label className="text-sm font-medium text-neutral-700 mb-1.5 block">
            Reason for rejection (applied to all)
          </label>
          <textarea
      ...
      >
        <p className="text-sm text-neutral-600">
          Are you sure? Categories with templates assigned to them cannot be
          deleted.
        </p>
      </Modal>
    </>
    ...
    <div>
      ...
      <div className="flex gap-6">
        ...
        <div className="flex-1 min-w-0">
          ...
          {(activeView === "templates" || activeView === "featured") &&
            renderTemplatesList()}
