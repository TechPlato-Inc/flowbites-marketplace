...
export function TemplateListingContent({
  ...
  const hasActiveFilters = activePlatform || activeQuery || activeCategory || activeTab;
        ...
        <div className="max-w-8xl mx-auto px-4 md:px-6 lg:px-8 py-8 sm:py-10">
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            <div>
              ...
              <p className="text-neutral-500 text-sm sm:text-base max-w-xl">
                Kickstart your next project with a premium Webflow, Framer, or Wix template built by
                world-class creators.
              </p>
            </div>
            <Link href="/dashboard/creator/upload-template" className="shrink-0">
          ...
          <div className="flex items-center gap-2 mt-5 overflow-x-auto pb-1 hide-scrollbar">
            ...
            {visibleCategories.map((cat) => (
              <button
                key={cat._id}
                onClick={() => setFilter('category', cat._id === activeCategory ? '' : cat._id)}
                className={`shrink-0 px-4 py-1.5 text-sm font-medium whitespace-nowrap transition-all border rounded-full ${
        ...
        <div className="max-w-8xl mx-auto px-4 md:px-6 lg:px-8 py-3 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div className="flex items-center gap-3 flex-wrap">
            <span className="text-sm text-neutral-500">
              <strong className="text-neutral-900">{totalCount.toLocaleString()}</strong> templates
              found
            </span>
            ...
            {activePlatform && (
              <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-neutral-200 text-xs font-medium text-neutral-700">
                ...
                <button onClick={() => setFilter('platform', '')} className="ml-0.5 hover:text-neutral-900">
                  <X size={12} />
                </button>
              </span>
            )}
            {activeQuery && (
              <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-neutral-200 text-xs font-medium text-neutral-700">
                ...
                <button onClick={clearSearch} className="ml-0.5 hover:text-neutral-900">
                  <X size={12} />
                </button>
              </span>
            )}
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            <form onSubmit={handleSearch} className="flex-1 sm:flex-none sm:w-52">
              <Input
                type="text"
                value={searchInput}
                ...
                inputSize="sm"
                className="!w-full !h-9 !text-sm !ring-1 !ring-transparent focus:!ring-primary-500"
              />
            </form>
      ...
      <div className="max-w-8xl mx-auto px-4 md:px-6 lg:px-8 py-6 sm:py-8">
        {templates.length === 0 ? (
          <div className="text-center py-20 sm:py-28">
            ...
            <h3 className="text-lg font-semibold text-neutral-900 mb-2">No templates found</h3>
