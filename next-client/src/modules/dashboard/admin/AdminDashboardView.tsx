...
import {
  CheckCircle, XCircle, ChevronDown, ChevronUp, ExternalLink, Plus,
  UserPlus, AlertCircle, Shield, MessageSquare, LayoutDashboard,
  FileText, ClipboardList, BookOpen, LayoutGrid
} from 'lucide-react';

type View = 'overview' | 'templates' | 'services' | 'disputes';
...
export const AdminDashboardView = () => {
  ...
  const handleReject = async () => {
    ...
    setRejecting(true);
    try {
      await api.post(`/admin/templates/${rejectTemplateId}/reject`, { reason: rejectReason.trim() });
      setPendingTemplates(prev => prev.filter(t => t._id !== rejectTemplateId));
  ...
  const handleReassign = async () => {
    ...
    setAssigning(true);
    try {
      const { data } = await api.patch(`/services/admin/orders/${assignOrderId}/reassign`, {
  ...
  const handleResolveDispute = async () => {
    ...
    setResolving(true);
    try {
      const { data } = await api.patch(`/services/admin/orders/${resolveOrderId}/resolve-dispute`, {
  ...
  const statusColor = (status: string) => {
    const map: Record<string, 'warning' | 'success' | 'error' | 'neutral' | 'info'> = {
      requested: 'warning', accepted: 'info', in_progress: 'info',
      delivered: 'success', completed: 'success', rejected: 'error',
      cancelled: 'neutral', revision_requested: 'warning', disputed: 'error',
    };
    return map[status] || 'neutral';
  };

  const unassignedCount = serviceOrders.filter(o => o.isGenericRequest && !o.assignedCreatorId).length;
  const disputeCount = serviceOrders.filter(o => o.status === 'disputed').length;
  ...
  const orderFilterTabs = [
    ...
    { key: 'requested', label: 'Requested', count: serviceOrders.filter(o => o.status === 'requested').length },
    { key: 'in_progress', label: 'In Progress', count: serviceOrders.filter(o => o.status === 'in_progress').length },
    { key: 'delivered', label: 'Delivered', count: serviceOrders.filter(o => o.status === 'delivered').length },
    { key: 'completed', label: 'Completed', count: serviceOrders.filter(o => o.status === 'completed').length },
  ];

  const navItems: NavItem[] = [
    { label: 'Overview', icon: LayoutDashboard, onClick: () => setActiveView('overview'), section: 'main' },
    { label: 'Template Approvals', icon: FileText, onClick: () => setActiveView('templates'), badge: pendingTemplates.length, section: 'Moderation' },
    { label: 'Service Orders', icon: ClipboardList, onClick: () => setActiveView('services'), badge: unassignedCount, section: 'Moderation' },
    { label: 'Disputes', icon: Shield, onClick: () => setActiveView('disputes'), badge: disputeCount, section: 'Moderation' },
    { label: 'Template Manager', icon: LayoutGrid, path: '/dashboard/admin/templates', section: 'Content' },
    { label: 'Creator Manager', icon: UserPlus, path: '/dashboard/admin/creators', section: 'Content' },
    { label: 'Blog Management', icon: BookOpen, path: '/dashboard/admin/blog', section: 'Content' },
  ];
    ...
    >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <span className="text-sm font-medium text-neutral-500">Pending Templates</span>
                <div className="w-9 h-9 bg-warning-light rounded-lg flex items-center justify-center">
                  <FileText size={18} className="text-warning" />
                </div>
              </div>
              <div className="text-2xl font-bold text-neutral-900">{pendingTemplates.length}</div>
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <span className="text-sm font-medium text-neutral-500">Unassigned Orders</span>
                <div className="w-9 h-9 bg-error-light rounded-lg flex items-center justify-center">
                  <UserPlus size={18} className="text-error" />
                </div>
              </div>
              <div className="text-2xl font-bold text-neutral-900">{unassignedCount}</div>
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <span className="text-sm font-medium text-neutral-500">Open Disputes</span>
                <div className="w-9 h-9 bg-error-light rounded-lg flex items-center justify-center">
                  <Shield size={18} className="text-error" />
                </div>
              </div>
              <div className="text-2xl font-bold text-neutral-900">{disputeCount}</div>
              <p className="text-xs text-neutral-400 mt-1">Require resolution</p>
            </div>

            <div className="bg-white border border-neutral-200 rounded-xl p-5">
              <div className="flex items-center justify-between mb-3">
                <span className="text-sm font-medium text-neutral-500">Platform Revenue</span>
              ...
              <div className="text-2xl font-bold text-neutral-900">${totalOrderRevenue.toFixed(2)}</div>
            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-neutral-900">
                  Pending Templates
                  <span className="ml-2 text-sm font-normal text-neutral-400">({pendingTemplates.length})</span>
                </h3>
                <Button
                  variant="ghost"
                  size="sm"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                {pendingTemplates.slice(0, 3).map((template) => (
                  <div key={template._id} className="bg-white border border-warning/30 rounded-xl overflow-hidden">
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex gap-2 mt-3">
                        <Button size="sm" className="flex-1" leftIcon={<CheckCircle size={14} />} onClick={() => handleApprove(template._id)}>
                          Approve
                        </Button>
                        <Button size="sm" variant="danger" className="flex-1" leftIcon={<XCircle size={14} />} onClick={() => openRejectModal(template._id)}>
                          Reject
                        </Button>
                      </div>
                    </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-neutral-900">
                  ...
                  <span className="ml-2 text-sm font-normal text-neutral-400">({unassignedCount})</span>
                </h3>
                <Button
                  variant="ghost"
                  size="sm"
                      ...
                      >
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="text-xs text-neutral-500 mt-0.5">
                            {buyer?.name || 'Buyer'} &middot; {new Date(order.createdAt).toLocaleDateString()}
                          </p>
                        </div>
                        <Button size="sm" leftIcon={<UserPlus size={14} />} onClick={() => openAssignModal(order._id)}>
                          Assign
                        </Button>
                      </div>
                    );
          ...
          {disputeCount > 0 && (
            <div>
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-neutral-900">
                  Open Disputes
                  <span className="ml-2 text-sm font-normal text-neutral-400">({disputeCount})</span>
                </h3>
                <Button
                  variant="ghost"
                  size="sm"
                    ...
                    return (
                      ...
                      >
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2">
                            <p className="text-sm font-medium text-neutral-900 truncate">{order.packageName}</p>
                            <Badge size="sm" variant="error">Disputed</Badge>
                          </div>
                          <p className="text-xs text-neutral-500 mt-0.5">
                            {buyer?.name || 'Buyer'} &middot; {order.dispute?.reason?.slice(0, 60) || 'No reason'}
                          </p>
                        </div>
                        <Button size="sm" variant="danger" onClick={() => openResolveModal(order._id)}>
                          Resolve
                        </Button>
                      </div>
                    );
      ...
      {activeView === 'templates' && (
        <div className="space-y-6">
          <h2 className="text-xl font-display font-bold text-neutral-900">
            Template Approvals
            {pendingTemplates.length > 0 && (
              <span className="ml-2 text-sm font-normal text-neutral-400">({pendingTemplates.length} pending)</span>
            )}
          </h2>
          ...
          {pendingTemplates.length === 0 ? (
            <div className="bg-white border border-neutral-200 rounded-xl p-12 text-center">
              ...
              <p className="text-neutral-600">No templates pending approval</p>
            </div>
          ) : (
                      ...
                      <div className="flex gap-5">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start justify-between mb-2">
                            ...
                            <div className="flex items-center gap-2">
                              <Badge variant="neutral" size="sm">{template.platform}</Badge>
                              <Badge size="sm">{template.category?.name || 'Uncategorized'}</Badge>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center gap-4 mb-4 text-sm text-neutral-500">
                            <span className="font-semibold text-neutral-900">${template.price}</span>
                            <span>Submitted {new Date(template.createdAt).toLocaleDateString()}</span>
                            {template.licenseType && (
                              <span className="capitalize">{template.licenseType} license</span>
                            )}
                          </div>
                          <div className="flex gap-2">
                            <Button size="sm" leftIcon={<CheckCircle size={14} />} onClick={() => handleApprove(template._id)}>
                              Approve
                            </Button>
                            <Button size="sm" variant="danger" leftIcon={<XCircle size={14} />} onClick={() => openRejectModal(template._id)}>
                              Reject
                            </Button>
                            <Button size="sm" variant="ghost" rightIcon={isExpanded ? <ChevronUp size={14} /> : <ChevronDown size={14} />} onClick={() => toggleExpand(template._id)}>
                    ...
                    {isExpanded && (
                      <div className="border-t border-neutral-200 bg-neutral-50 p-5">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div>
                            <h4 className="text-xs font-semibold text-neutral-400 uppercase mb-2">Full Description</h4>
                            <p className="text-sm text-neutral-700 whitespace-pre-wrap">{template.description}</p>
                          </div>
                          <div className="space-y-4">
                            {template.demoUrl && (
                              <div>
                                <h4 className="text-xs font-semibold text-neutral-400 uppercase mb-1">Demo URL</h4>
                                <a href={template.demoUrl} target="_blank" rel="noopener noreferrer" className="text-primary-600 hover:underline flex items-center gap-1 text-sm">
                                  {template.demoUrl} <ExternalLink size={12} />
                                </a>
                              </div>
                            )}
                            <div>
                              <h4 className="text-xs font-semibold text-neutral-400 uppercase mb-1">Tags</h4>
                        ...
                        {template.gallery && template.gallery.length > 0 && (
                          <div className="mt-5">
                            <h4 className="text-xs font-semibold text-neutral-400 uppercase mb-2">Gallery</h4>
                            <div className="flex gap-3 overflow-x-auto pb-2">
                              {template.gallery.map((img, i) => (
                                <img key={i} src={getUploadUrl(`images/${img}`)} alt={`Gallery ${i + 1}`} className="w-36 h-24 object-cover rounded-lg border border-neutral-200 flex-shrink-0" />
                              ))}
                            </div>
                          </div>
                        )}
      ...
      {activeView === 'services' && (
        <div className="space-y-6">
          ...
          {servicesLoading ? (
            <div className="text-center py-12 text-neutral-500">Loading service orders...</div>
          ) : filteredOrders.length === 0 ? (
            <div className="bg-white border border-neutral-200 rounded-xl p-12 text-center">
              <ClipboardList size={40} className="text-neutral-300 mx-auto mb-3" />
              <p className="text-neutral-600">No service orders found</p>
            </div>
          ) : (
            <div className="space-y-3">
              {filteredOrders.map((order) => {
                ...
                const templateInfo = order.templateId && typeof order.templateId === 'object' ? order.templateId : null;
                const buyer = typeof order.buyerId === 'object' ? order.buyerId : null;
                ...
                return (
                  <div
                    ...
                    className={`bg-white border rounded-xl p-4 transition-colors ${
                      isUnassigned ? 'border-warning/40' : order.status === 'disputed' ? 'border-error/40' : 'border-neutral-200'
                    }`}
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          {isGeneric && (
                            <Badge variant="warning" size="sm">Custom Request</Badge>
                          )}
                          <span className="text-xs font-mono text-neutral-400">{order.orderNumber}</span>
                        </div>

                        <h4 className="font-semibold text-neutral-900 mb-1">{order.packageName}</h4>
                        ...
                        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-neutral-500">
                          <span>Buyer: <strong className="text-neutral-700">{buyer?.name || 'Unknown'}</strong></span>
                          {order.price > 0 ? (
                            <span>Price: <strong className="text-neutral-700">${order.price}</strong></span>
                          ) : isGeneric ? (
                            <span className="text-warning font-medium">Price: TBD</span>
                          ) : null}
                        ...
                        <p className="text-xs text-neutral-500 mt-1.5 line-clamp-1">{order.requirements}</p>
                      </div>
                      ...
                      <div className="flex flex-col gap-1.5 shrink-0">
                        ...
                        {!isUnassigned && order.status !== 'disputed' && (
                          <Button size="sm" variant="outline" leftIcon={<UserPlus size={14} />} onClick={() => openAssignModal(order._id)}>
                            Reassign
                          </Button>
                        )}
                        <Link href={`/service-orders/${order._id}`}>
                          <Button size="sm" variant="ghost" leftIcon={<MessageSquare size={14} />} className="w-full">
                            View
                          </Button>
                        </Link>
                      </div>
      ...
      {activeView === 'disputes' && (
        <div className="space-y-6">
          <h2 className="text-xl font-display font-bold text-neutral-900">
            Disputes
            {disputeCount > 0 && (
              <span className="ml-2 text-sm font-normal text-neutral-400">({disputeCount} open)</span>
            )}
          </h2>

          {(() => {
            const disputed = serviceOrders.filter(o => o.status === 'disputed');
            return disputed.length === 0 ? (
              <div className="bg-white border border-neutral-200 rounded-xl p-12 text-center">
                <Shield size={40} className="text-success mx-auto mb-3" />
                <p className="text-neutral-600">No open disputes</p>
              </div>
            ) : (
              <div className="space-y-3">
                {disputed.map((order) => {
                  ...
                  return (
                    <div key={order._id} className="bg-white border border-error/30 rounded-xl overflow-hidden">
                      <div className="p-5">
                        <div className="flex items-start justify-between gap-4">
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1">
                            ...
                            {dispute && (
                              <div className="bg-error/5 border border-error/15 rounded-lg p-3">
                                <p className="text-sm font-medium text-neutral-800 mb-1">Dispute Reason:</p>
                                <p className="text-sm text-neutral-700">{dispute.reason}</p>
                                <p className="text-xs text-neutral-500 mt-1.5">
                                  Opened {new Date(dispute.openedAt).toLocaleDateString()} by {typeof dispute.openedBy === 'object' ? dispute.openedBy.name : 'buyer'}
                                </p>
                              </div>
                            )}
                          </div>
                          <div className="flex flex-col gap-1.5 shrink-0">
                            <Button size="sm" onClick={() => openResolveModal(order._id)}>
                              Resolve
                            </Button>
                            <Link href={`/service-orders/${order._id}`}>
                              <Button size="sm" variant="ghost" leftIcon={<MessageSquare size={14} />} className="w-full">
      ...
      <Modal
        ...
        footer={
          <div className="flex gap-3">
            ...
            <Button onClick={handleReassign} isLoading={assigning} disabled={!assignCreatorId}>
              Assign & Accept
            </Button>
          </div>
        }
        ...
        <div className="space-y-4">
          <div className="bg-primary-50 rounded-lg p-3 flex items-start gap-2">
            ...
            <p className="text-sm text-primary-800">
              Assigning a creator will move the order to &quot;accepted&quot; status.
            </p>
          </div>

          <div>
            <label className="block text-sm font-medium text-neutral-700 mb-1">Select Creator *</label>
            <select
            ...
            >
              ...
              {creators.map((c) => (
                <option key={c._id} value={c._id}>
                  {c.name} ({c.email}) {c.role === 'admin' ? '- Flowbites Team' : ''}
                </option>
              ))}
            </select>
          </div>
          ...
          <div>
            <label className="block text-sm font-medium text-neutral-700 mb-1">Set Price (optional)</label>
            <div className="relative">
            ...
            <p className="text-xs text-neutral-500 mt-1">30% platform fee applies. Creator receives 70%.</p>
          </div>
        </div>
      </Modal>
      ...
      <Modal
        ...
        footer={
          <div className="flex gap-3">
            ...
            <Button onClick={handleResolveDispute} isLoading={resolving} disabled={!resolveResolution.trim() || !resolveOutcome}>
              Resolve Dispute
            </Button>
          </div>
        }
        ...
        <div className="space-y-4">
          <div>
            ...
            >
              ...
              <option value="refund">Full Refund to Buyer (cancel order)</option>
              <option value="release_payment">Release Payment to Creator (complete order)</option>
              <option value="partial_refund">Partial Refund (complete order + refund difference)</option>
              <option value="redo">Redo Work (send back to creator)</option>
            </select>
          </div>

          <div>
            <label className="block text-sm font-medium text-neutral-700 mb-1">Resolution Details *</label>
            <textarea
