...
import {
  LayoutDashboard, FileText, Users, Plus, Search, Trash2, Edit3,
  Eye, Star, StarOff, ArrowLeft, Save, ChevronLeft, ChevronRight,
...
const CATEGORIES = [
  'Web Design', 'Webflow', 'Framer', 'Wix', 'No-Code',
  'Business', 'Tutorials', 'Trends', 'SEO', 'Freelancing',
];
...
export function BlogManagementView() {
  ...
  const fetchWriters = async () => {
    ...
    try {
      ...
      allPosts.forEach(post => {
        ...
        const authorName = typeof post.author === 'object' ? post.author.name : post.authorName;
        const authorEmail = typeof post.author === 'object' ? (post.author as any).email || '' : '';
        ...
        if (writerMap.has(authorId)) {
          ...
          if (!existing.lastPost || new Date(post.createdAt) > new Date(existing.lastPost)) {
      ...
      setWriters(Array.from(writerMap.values()).sort((a, b) => b.postCount - a.postCount));
    } catch (error) {
  ...
  const computeStats = useCallback(async () => {
    try {
      const { data } = await api.get('/blog/admin/all?limit=1000');
      const allPosts: BlogPost[] = data.data.posts;
      const authorIds = new Set(allPosts.map(p => typeof p.author === 'object' ? p.author._id : p.author));
  ...
  const toggleFeatured = async (post: BlogPost) => {
    try {
      ...
      setPosts(prev =>
        prev.map(p => p._id === post._id ? { ...p, isFeatured: !p.isFeatured } : p)
      );
    } catch (error) {
  ...
  const resetForm = () => {
    setFormData({
      title: '', slug: '', excerpt: '', content: '', category: 'Web Design',
      tags: '', status: 'draft', metaTitle: '', metaDescription: '',
      isFeatured: false, authorName: '', authorRole: 'Contributor',
    });
  ...
  const navItems: NavItem[] = [
    ...
    { label: 'All Posts', icon: FileText, onClick: () => setActiveView('posts'), badge: stats.totalPosts, section: 'Blog' },
    { label: 'Writers', icon: Users, onClick: () => setActiveView('writers'), badge: stats.totalWriters, section: 'Blog' },
  ];
        ...
        <div className="space-y-8">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <span className="text-sm font-medium text-neutral-500">Total Posts</span>
                <div className="w-9 h-9 bg-primary-50 rounded-lg flex items-center justify-center">
                  <FileText size={18} className="text-primary-500" />
                </div>
              </div>
              <div className="text-2xl font-bold text-neutral-900">{stats.totalPosts}</div>
              <div className="flex items-center gap-3 mt-2 text-xs text-neutral-500">
                <span className="text-success font-medium">{stats.publishedPosts} published</span>
                <span className="text-warning font-medium">{stats.draftPosts} drafts</span>
              </div>
            </div>
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <span className="text-sm font-medium text-neutral-500">Writers</span>
                <div className="w-9 h-9 bg-info-light rounded-lg flex items-center justify-center">
                  <Users size={18} className="text-info" />
                </div>
              </div>
              <div className="text-2xl font-bold text-neutral-900">{stats.totalWriters}</div>
              <p className="text-xs text-neutral-400 mt-1">{stats.featuredPosts} featured posts</p>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <Button
                variant="ghost"
                onClick={() => { setStatusFilter('draft'); setActiveView('posts'); }}
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-neutral-500">{stats.draftPosts} posts awaiting review</p>
                </div>
              </Button>
              <Button
                variant="ghost"
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-neutral-500">{stats.totalWriters} active writers</p>
                </div>
              </Button>
            </div>
          </div>
              ...
              >
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-neutral-500">
                    {post.authorName} &middot; {post.category} &middot; {new Date(post.createdAt).toLocaleDateString()}
                  </p>
                </div>
                <div className="flex items-center gap-1 ml-3">
                  <span className="text-xs text-neutral-400 mr-2">
                    <Eye size={12} className="inline mr-0.5" />{post.stats?.views || 0}
                  </span>
                  <Button size="sm" variant="ghost" onClick={() => startEdit(post._id)}>
                    <Edit3 size={14} />
                  </Button>
                </div>
              </div>
      ...
      {activeView === 'posts' && (
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-display font-bold text-neutral-900">All Blog Posts</h2>
            <Button size="sm" leftIcon={<Plus size={16} />} onClick={startCreate}>
              New Post
            </Button>
          </div>
                ...
                {posts.map((post, idx) => (
                  ...
                  >
                    ...
                    <div className="md:col-span-5 min-w-0">
                      <div className="flex items-center gap-2">
                        <h4 className="text-sm font-medium text-neutral-900 truncate">{post.title}</h4>
                        {post.isFeatured && <Star size={12} className="text-warning fill-warning shrink-0" />}
                      </div>
                      <p className="text-xs text-neutral-400 truncate mt-0.5">/blog/{post.slug}</p>
                    </div>

                    {/* Author */}
                    <div className="md:col-span-2 min-w-0">
                      <p className="text-sm text-neutral-700 truncate">{post.authorName}</p>
                    ...
                    {/* Category */}
                    <div className="md:col-span-1">
                      <span className="text-xs text-neutral-600">{post.category}</span>
                    </div>

                    {/* Views */}
                    <div className="md:col-span-1">
                      <span className="text-sm text-neutral-700">{(post.stats?.views || 0).toLocaleString()}</span>
                    </div>
                    ...
                    <div className="md:col-span-2 flex items-center justify-end gap-1">
                      <button
                        ...
                        title={post.isFeatured ? 'Remove featured' : 'Make featured'}
                      >
                        {post.isFeatured ? (
                      ...
                      <button
                        onClick={() => { setDeletePostId(post._id); setDeleteModalOpen(true); }}
      ...
      {(activeView === 'create' || activeView === 'edit') && (
        <div className="space-y-6">
          <div className="flex items-center gap-3">
            <button
              onClick={() => { resetForm(); setActiveView('posts'); }}
            ...
            <div>
              ...
              <p className="text-sm text-neutral-500">
                {activeView === 'create' ? 'Write and publish a new blog post' : `Editing: ${editingPost?.title}`}
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-5">
              ...
              <div className="bg-white border border-neutral-200 rounded-xl p-5 space-y-4">
                ...
                <div>
                  ...
                  />
                  <p className="text-xs text-neutral-400 mt-1">{formData.excerpt.length}/500</p>
                </div>
              </div>

              {/* Content */}
              <div className="bg-white border border-neutral-200 rounded-xl p-5">
                <label className="block text-sm font-medium text-neutral-700 mb-1.5">Content * (HTML supported)</label>
                <textarea
              ...
              <div className="bg-white border border-neutral-200 rounded-xl p-5 space-y-4">
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (!formData.metaTitle && formData.title) {
                        handleFormChange('metaTitle', `${formData.title} — Flowbites`.slice(0, 70));
                      }
                      if (!formData.metaDescription && formData.excerpt) {
                        handleFormChange('metaDescription', formData.excerpt.slice(0, 160));
                      }
                    }}
                ...
                <div>
                  ...
                  />
                  <p className="text-xs text-neutral-400 mt-1">{formData.metaTitle.length}/70</p>
                </div>
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full px-4 py-3 border border-neutral-300 rounded-lg focus:border-primary-500 focus:ring-2 focus:ring-primary-500 focus:outline-none resize-none text-sm"
                  />
                  <p className="text-xs text-neutral-400 mt-1">{formData.metaDescription.length}/160</p>
                </div>
                {/* SEO Preview */}
                <div className="border border-neutral-200 rounded-lg p-3 bg-neutral-50">
                  <p className="text-xs text-neutral-400 mb-2 font-semibold uppercase">Search Preview</p>
                  <p className="text-[#1a0dab] text-sm font-medium truncate">
                    {formData.metaTitle || formData.title || 'Post Title'} — Flowbites
                  </p>
            ...
            <div className="space-y-5">
              {/* Publish Settings */}
              <div className="bg-white border border-neutral-200 rounded-xl p-5 space-y-4">
                <h3 className="text-sm font-semibold text-neutral-900">Publish</h3>
                ...
                <label className="flex items-center gap-3 cursor-pointer">
                  ...
                  />
                  <span className="text-sm text-neutral-700">Featured post</span>
                </label>
              ...
              <div className="bg-white border border-neutral-200 rounded-xl p-5 space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-neutral-700 mb-1.5">Tags</label>
                  <input
                    type="text"
                    value={formData.tags}
              ...
              {activeView === 'create' && (
                <div className="bg-white border border-neutral-200 rounded-xl p-5 space-y-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-neutral-700 mb-1.5">Author Name *</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-neutral-700 mb-1.5">Author Role</label>
                    <input
                      type="text"
              ...
              <div className="bg-white border border-neutral-200 rounded-xl p-5 space-y-4">
                ...
                <div className="border-2 border-dashed border-neutral-300 rounded-lg p-6 text-center">
                  {coverImageFile ? (
                    <div>
                      <p className="text-sm text-neutral-700 font-medium">{coverImageFile.name}</p>
                      <button
                    ...
                    <>
                      <p className="text-sm text-neutral-500 mb-2">Upload cover image</p>
      ...
      {activeView === 'writers' && (
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-display font-bold text-neutral-900">
              Writers
              <span className="ml-2 text-sm font-normal text-neutral-400">({writers.length})</span>
            </h2>
          </div>

          {writersLoading ? (
            <div className="text-center py-12 text-neutral-500">Loading writers...</div>
          ) : writers.length === 0 ? (
            <div className="bg-white border border-neutral-200 rounded-xl p-12 text-center">
              <Users size={40} className="text-neutral-300 mx-auto mb-3" />
              <p className="text-neutral-600">No writers found</p>
              <p className="text-sm text-neutral-400 mt-1">Writers will appear here after they create blog posts</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {writers.map((writer) => (
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className="font-semibold text-neutral-900 truncate">{writer.name}</h4>
                  ...
                  <div className="grid grid-cols-3 gap-3 mt-4 pt-4 border-t border-neutral-100">
                    <div className="text-center">
                      <div className="text-lg font-bold text-neutral-900">{writer.postCount}</div>
                      <div className="text-xs text-neutral-400">Posts</div>
                    </div>
                    <div className="text-center">
                      <div className="text-lg font-bold text-neutral-900">{writer.totalViews.toLocaleString()}</div>
                      <div className="text-xs text-neutral-400">Views</div>
                    </div>
                    <div className="text-center">
                      <div className="text-sm font-medium text-neutral-700">
                        {writer.lastPost ? new Date(writer.lastPost).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : 'N/A'}
                      </div>
