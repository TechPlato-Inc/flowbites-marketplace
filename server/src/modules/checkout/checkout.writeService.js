...
import { AffiliateService } from '../affiliates/affiliate.service.js';
...
export class CheckoutWriteService {
  async createTemplateCheckoutSession(buyerId, buyerEmail, items, couponCode, referralCode) {
    ...
    // Track purchase_start event (non-blocking)
    analyticsService.trackEvent('purchase_start', buyerId, {
      orderId: order._id.toString(),
      orderNumber: order.orderNumber,
      total: order.total,
      itemCount: orderItems.length,
  ...
  async _handleStripeRefund(charge) {
    ...
    // Emit order refunded domain event (Stripe-initiated refund; no adminId)
  ...
  async fulfillTemplateOrder(orderId, chargeId) {
    ...
    // Track purchase_success event (non-blocking)
    analyticsService.trackEvent('purchase_success', order.buyerId, {
      orderId: order._id.toString(),
      orderNumber: order.orderNumber,
      total: order.total,
    ...
    // Record affiliate conversion if a referral code was attached (non-blocking)
    if (order.referralCode) {
      affiliateService.recordConversion(
        order._id, order.buyerId, order.total, order.referralCode
      ).catch(err => console.error('Failed to record affiliate conversion:', err));
    }

    // Emit order paid domain event
